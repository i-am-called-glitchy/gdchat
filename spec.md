...
- 4000 - Bad authorization, client supplied an invalid jwt.
- 4001 - Hard rate limit, client spammed too many requests. Server must in the close frame specify a close message in json, with a `retry_after` key in seconds.
- 4002 - Client does not have an extension that is absolutely required. Please don't do this if you don't have to.
- 4003 - Client did not authenticate quickly enough.

## About extensions

Extensions should advertise themselves cleanly, and if not required for the connection they should not introduce breaking changes into the protocol. For example:

Say there's a profile picture extension, to the [full user object](#full-user) the extension would probably append a key like `avatar`, mirroring discord's system (cmon you gotta admit their api does work, may as well yoink some ideas)

I intentionally didn't include base profile changes, permissions, etc.., that's for either admin bots or extensions.

## About mentions

Mentions are when a message containing "<@user uuid>" is sent, in this case the "mentions" field must be populated with the user's message.
...
Note: Namespace exists because there can be multiple servers and this should help with caching. Version must be incremented for every profile update, and logic basically becomes `if (cached.version < partial.version) refetch()`. I'm not implementing federation into base, but yall can cook that up with extensions.

### Full user

```jsonc
{"id": "UUID HERE", "dname": "Display Name", "uname": "unique_name", "namespace": "amcalledglitchy.dev", "ver": 1}
```

#### Display name restrictions:

- 1-40 characters
- Explicitly most visible unicode characters allowed. The space character is an exception.
- No leading/trailing whitespace.
- Server is allowed to use custom logic to block lookalikes like U+0430 and U+0061.
...
```jsonc
{
    ...
    "timestamp": 1234, // in unix epoch seconds
    "mentions": [
        /* partial users here */
    ]
}
...
### Notes
- S and C in the Direction column mean Server to client and Client to server respectively
- NR is an abbreviation for Nonce Required, which states whether whoever sent the message has to add a nonce parameter, which is just a random string 
- In the NR field: x means no, ? means on some edge cases, clarified in description, + means yes
- A nonce is a randomly generated string the client sends alongside a packet so that the server can identify it with a reply and aid client implementations.
...
{"data": {
    ...
    "ext": [] // Array of strings, these are identifiers for extensions, like profile pictures, etc..
}}
```
...
```jsonc
{"data":{
    "jwt": "eyJh...omSE", // Authentication token the client got
    "ext": [] // Array of strings, extensions the client supports, this is to cooperate in case server wants to kick us with 4002
}}
```

- Note: If the client fails authentication, immediately close with 4000 ([Relevant](#websocket-application-layer-close-codes)), do not send an error nor ready packet;

On success:

Send an [OK](#ok) packet with the following structure, and send a CHANNELS packet.
...
Should be sent whenever a request succeeds. Properly document what each response_type expects in the data field.
...
Notes:
- Notation for code and type is `TYPE/CODE`, as in `SERVER/OVERLOADED`.
...
Note: If the client is referencing something they should not have access to, like a channel that exists but they don't have permissions to even see it, return `INVALID/NOT_FOUND` for security reasons.

|Code|Explaination|
|-|-|
|NOT_FOUND|A resource (like a channel, a message, etc..) was not found.|
|BAD_OP|The opcode the client used does not exist in the server's implementation.|
...
|SAME_MSG_NONCE|Server recieved multiple message with the same nonce.|
...
|OVERLOADED|Server is being overloaded and is locking down specific API endpoints to prevent a full stall.|
...
Return error `INVALID/NOT_FOUND` if the client cannot see the channel being referenced or should not be able to. Return error `INVALID/BAD_SUB_TYPE` if the type field does not match.
...
By default the behaviour should be to have type on full.
...
> try not to run into any memory issues while processing this client devs ;3 (you're welcome for partial)
...
`before` and `after` should not be present, if this happens the server should reject the request and return a `INVALID/EXCLUSIVE_BEFORE_AFTER` error. If the channel can't be seen or does not exist return the `INVALID/NOT_FOUND` error.
...
Server should return the `GENERIC/PERMISSION` error if the message is not from the user AND the user does not have administrator permissions. Otherwise, the server should delete the message from the database (or just never send it again, flag it in the db, and nobody would know ;3 (also useful for moderation ig)) and give a base OK.
...
{"data":{
    "channel": "UUID HERE",
    "content": "hello :3",
    "msgnonce": 1234 // client generates an integer
}}
```

The scope of msgnonce should be per-websocket, however the server should drop them over time gradually if memory is a strict constraint. msgnonce must be a 32 bit unsigned integer (have enough choices devs?).

If the channel does not exist or the user can't see it, return `INVALID/NOT_FOUND`, if the message is blank except for whitespace, return `INVALID/EMPTY_MESSAGE`, if the message is too long, return `INVALID/MESSAGE_TOO_LONG`. If the server gets multiple SEND packets with the same msgnonce, the server should return the error `INVALID/SAME_MSG_NONCE`.
...
Server should also send the resulting MSG packet to the client, and to all subscribed clients watching the channel, (of course obeying [subscription types](#subscription-types))

Read [this](#about-mentions)
...
Note: See [this](#partial-user) and [this](#message-object).
